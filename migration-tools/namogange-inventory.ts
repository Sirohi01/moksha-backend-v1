...
// MIGRATION_NAMOGANGE_MONGO_URI expected format: mongodb://user:pass@host:port/namogange_db
...
async function runInventory() {
  ...
  const collections = ['members', 'volunteers'];
      ...
      const mobileDuplicates = await coll.aggregate([
        { $match: { mobile: { $exists: true, $nin: [null, ''] } } },
        {
          $group: {
            _id: '$mobile',
            count: { $sum: 1 },
            docs: { $push: { _id: '$_id', name: { $ifNull: ['$applicantName', { $ifNull: ['$name', 'Unknown'] }] } } }
          }
        },
        { $match: { count: { $gt: 1 } } },
        { $limit: 10 }
